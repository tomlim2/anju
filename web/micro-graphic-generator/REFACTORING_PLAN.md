...
| Phase 5. Validator 분리 | 구현 완료 / 최종 gate 대기 | read-only rule registry와 structured result를 도입하고 앱만 validation metadata를 기록하게 했다. Random 1,000회 soak가 남아 있다. |
| Phase 6. Catalog/export/app shell 분리 | 구현 완료 / assertion 보강 대기 | Compose catalog, PNG/SVG export, app orchestration을 분리하고 `index.html` inline script를 제거했다. Export 전후 PRNG 불변 assertion 보강이 남아 있다. |
| Phase 7. 정리와 문서 갱신 | 진행 중 | legacy `contentPanel` 코드와 중복 구현은 제거했다. README 모듈 설명, extension point 문서, 최종 시각 QA는 남아 있다. |

### 현재 검증 결과

- `npm run test:generator`: 통과
- Node pure/architecture test: 13개 통과
- Playwright browser test: 12개 통과
- Random generation: 100회 연속 violation 0
...
- `npm run test:generator:soak`로 Random 1,000회 최종 gate를 실행한다.
- PNG/SVG export 전후 PRNG state 불변을 독립 assertion으로 보강한다.
- `1440x900`, `900x1200`, `390x844`에서 대표 generator/Compose 상태를 시각 검토한다.
- README의 `단일 파일`, `Layout renderer`, `contentPanel` 설명을 현재 module naming map으로 전면 갱신한다.
- 최종 `git diff --check`, 전체 test, soak test 후 refactoring 완료 상태로 문서를 갱신한다.
...
아래 구조는 localhost 실행이 공식 계약으로 승인됐을 때의 목표다.
...
```json
{
  "scripts": {
    ...
    "test:generator": "node web/micro-graphic-generator/tests/run-tests.mjs --iterations=100",
    "test:generator:soak": "node web/micro-graphic-generator/tests/run-tests.mjs --iterations=1000"
  },
  "devDependencies": {
...
`playwright.config.mjs`의 `webServer.command`는 `node web/micro-graphic-generator/tests/static-server.mjs --host=127.0.0.1 --port=4191`로 고정하고 `reuseExistingServer: false`를 사용한다. browser test의 base URL은 `http://127.0.0.1:4191/web/micro-graphic-generator/`이며 test runner가 server 시작과 종료를 소유한다.
...
```text
app
├── catalog-renderer → token-model + vocabulary + svg
├── grid-layout → config + random
├── grid-selection → grid-layout + token-model + vocabulary + typography
├── grid-renderer → svg + graphics + typography
├── grid-finalizer → config + typography + svg
├── validation → config + typography
└── export → config + svg
```
...
- 현재 README의 `Random block grid layer`는 migration 중에만 쓰는 umbrella term이다.
- `grid-layout`: cell, footprint, packing, anchor 관계만 계산한다. README의 `Layout renderer` 경계 중 geometry 책임에 해당한다.
...
- legacy `Content block`과 `Content detail`은 grid module 이름에 재사용하지 않는다.
...
- root manifest와 lockfile에 pinned Playwright dev dependency와 세 generator script가 기록돼 있다.
