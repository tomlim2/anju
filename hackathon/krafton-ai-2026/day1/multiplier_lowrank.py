"""
MultiplierBoard — Low-Rank Factorized Transformer
SS1: Key experiment for minimal P_2
"""
...
class LowRankLinear(nn.Module):
    """W = V @ U, rank r << min(in, out). Params: in*r + r*out + out(bias)."""
...
class LowRankMultiplyTransformer(nn.Module):
    def __init__(self, d_model=32, n_heads=2, n_layers=2, d_ff=64, rank=2):
        super().__init__()
        self.d_model = d_model
        self.seq_len = 24
        ...
        self.layers = nn.ModuleList([
            LowRankBlock(d_model, n_heads, d_ff, rank=rank)
...
# ============================================================
# Experiments — run one at a time (MPS)
# ============================================================
...
if __name__ == "__main__":
    ...
    # Select experiment via CLI arg
    exp = int(sys.argv[1]) if len(sys.argv) > 1 else 1

    configs = {
        1: ("EXP-LR1", 32, 2, 2, 64, 2),   # d=32, rank=2
        2: ("EXP-LR2", 32, 2, 2, 64, 1),   # d=32, rank=1
        3: ("EXP-LR3", 24, 2, 2, 48, 2),   # d=24, rank=2
        4: ("EXP-LR4", 24, 2, 2, 48, 1),   # d=24, rank=1
        5: ("EXP-LR5", 16, 2, 2, 32, 2),   # d=16, rank=2
    }

    if exp not in configs:
        print(f"Usage: python3 multiplier_lowrank.py [1-5]")
        print("Configs:")
        for k, (name, d, l, h, ff, r) in configs.items():
            print(f"  {k}: {name} d={d} L={l} H={h} ff={ff} rank={r}")
        sys.exit(1)

    name, d, n_layers, n_heads, d_ff, rank = configs[exp]

    print(f"\n{'='*60}")
    print(f"{name}: LowRank d={d}, L={n_layers}, H={n_heads}, ff={d_ff}, rank={rank}")
    print(f"{'='*60}")
    ...
    model = LowRankMultiplyTransformer(
        d_model=d, n_heads=n_heads, n_layers=n_layers, d_ff=d_ff, rank=rank
    )
    ...
    print(f"\nQuick eval (5K): {acc:.4f}")

    # Full eval if promising
    if acc >= 0.95:
        ...
        if acc_full >= 0.99:
            ...
            print(f"Saved: best_model_{name}.pt")
    else:
        acc_full = acc

    print(f"\n=== RESULT ===")
    print(f"{name} | d={d} L={n_layers} H={n_heads} ff={d_ff} rank={rank} | params={p} | acc={acc_full:.4f}")
