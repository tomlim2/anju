...
- menagerie 프레임 비용(draw call·재질)을 재고 줄이는 규칙은? → [`guidelines/performance.md`](../web/menagerie/guidelines/performance.md)
- menagerie 그리기 리팩토링이 형태를 안 바꿨는지 확인하려면? → `node web/menagerie/scripts/drawdiff.mjs` ([determinism.md](../web/menagerie/guidelines/determinism.md))
