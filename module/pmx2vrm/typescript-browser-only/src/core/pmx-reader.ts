 ...
 *
 * No file I/O — accepts raw bytes + texture map + deps injection.
 */

import { extname } from "./path-utils.js";
import { decodeTga, decodeBmp } from "./image-decoders.js";
import type { PmxBone, PmxMaterial, PmxMorph, PmxMorphOffset, PmxRigidBody, PmxJoint, PmxData, ConvertDeps } from "./types.js";
...
/**
 * Load a texture from in-memory map and convert to PNG via deps.
 */
async function loadTextureAsPng(
  data: Uint8Array,
  ext: string,
  deps: ConvertDeps,
): Promise<Uint8Array> {
  ...
  if (e === ".tga") {
    ...
    return deps.image.rgbaToPng(decoded.data, decoded.width, decoded.height);
  }
  ...
  // PNG, JPEG, etc. — delegate to adapter
  return deps.image.toPng(data, ext);
}

/**
 * Resolve texture from the in-memory map.
...
function resolveTexture(
  ...
  // Exact match
  if (textures.has(normalized)) return textures.get(normalized)!;
...
/**
 * Read PMX binary and return normalized data for the converter pipeline.
 *
 * @param pmxBytes - Raw PMX file bytes
 * @param textures - Map of relative texture path → file bytes
 * @param deps - Image/text codec adapters
 * @param scale - Coordinate scale factor (default 0.08)
 */
export async function read(
  ...
  // Textures: load + convert to PNG via deps
  const loadedTextures: Uint8Array[] = [];
  const textureMimes: string[] = [];

  for (const texPath of raw.texture_paths) {
    const texData = resolveTexture(textures, texPath);
    if (texData) {
      try {
        const ext = extname(texPath);
        const pngBuffer = await loadTextureAsPng(texData, ext, deps);
        loadedTextures.push(pngBuffer);
    ...
    // Fallback
    const fallback = await deps.image.createFallback();
