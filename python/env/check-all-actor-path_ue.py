...
paths_to_check = ["/Game/CineProps/", "/Game/CineMaps/"]
...
def check_materials_in_sm_actor (static_mesh_component):
	issued_materials = []
...
def format_error_message (form):
	clean_path = clean_asset_path(form['path'])
	return f"[{form['key']}] '{form['name']}' {form['issue']['message']} {clean_path}"

def format_material_lines(material):
	lines = []
...
def format_material_texture_issue_actor(actor):
	lines = []
	if actor['materials']:
...
def issued_list_to_message(actors):
	lines = []
	for actor in actors:
		lines.extend(format_material_texture_issue_actor(actor))
	return "\n".join(lines)

def process_static_mesh_actor(actor):
