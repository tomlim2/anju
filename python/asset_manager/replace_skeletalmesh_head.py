...
datatable_path = "/Script/Engine.DataTable'/Game/Character/Anime/_LookDev/DataTable/DT_LookDevHead.DT_LookDevHead'"
da_cm_parameter_name = "CustomMaterialsHead"
skeletal_mesh_parameter_name = "BaseSkeletalMesh"
skeletal_mesh_parameter_name_part = "SK_AnimePartHead"
skeletal_mesh_to_replace = "/Script/Engine.SkeletalMesh'/Game/Character/Anime/Female/Body/004_Bleue/Skinning/Bleue_head.Bleue_head'"

def get_material_list_from_selected_skeletalmesh(selected_asset):
    get_material_list = []

    # For head, get materials that have "BkA" OR "SrA" OR "BeA" in display name
    skeletal_materials = selected_asset.get_editor_property('materials')
    if skeletal_materials:
        for sm in skeletal_materials:
            ...
            if mat_interface:
                ...
                if "BkA" in mat_name or "SrA" in mat_name or "BeA" in mat_name or "GnA" in mat_name or "GnB" in mat_name or "WeA" in mat_name or "BnA" in mat_name or "BnB" in mat_name or "GyA" in mat_name:
                    get_material_list.append(mat_interface)
                    print(f"Found material: {mat_name} ({mat_interface.get_path_name()})")

    print(f"Total materials found: {len(get_material_list)}")
						...
						if new_sk_mesh:
							...
							if not discovered_columns:
								...
								possible_columns = [
									...
									'BaseSkeletalMesh', 'SK_AnimePartCloth', 'SK_AnimePart', 'OutlineMaterial',
									'Texture', 'TextureSet', 'MaterialSet', 'bIsFemale',
...
if len(selected_assets) == 1:
	...
	if not material_list:
		print("No valid materials found on selected SkeletalMesh. Aborting.")
		raise SystemExit
